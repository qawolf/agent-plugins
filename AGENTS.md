...
Load `skills/qawolf/SKILL.md` for shared connection and safety rules. Install the sibling `qawolf-flow-outline`, `qawolf-flow-maintenance`, `qawolf-onboarding` and `qawolf-trigger-setup` skills too. A project may keep these under `.agents/skills/` or its client's native skill location.
