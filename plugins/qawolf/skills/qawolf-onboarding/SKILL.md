...
The first question you ask is the moment you launch. Put `runner_launch` in the same tool block as it, under an id you keep, and leave it running.
...
Pass the application target, proposed or user-selected journey, selection rationale, that this is the application's first flow, known workspace and environment, the id of the runner you already launched, approved access references, permission boundaries, prior observations, and unresolved gaps. Do not pass secret values or invent missing context. Flow Outline explores under that runner rather than starting a second one.

Pass the finish state as settled, not open: activate the flow once its run passes. Nothing here has explained activation to this user, so it is not theirs to decide.
