...
Skills supply instructions; MCP supplies tools. Some clients need separate setup for each. These configurations follow client documentation, not authenticated tests of every client.
...
Review the adapter before enabling it. It registers all five QA Wolf skills, not MCP. Merge the server into the active profile's `~/.hermes/config.yaml`:
...
No executable plugin is needed. Copy all five sibling skill directories to project `.agents/skills/` or user `~/.config/opencode/skills/`. Merge into `opencode.json` or user `~/.config/opencode/opencode.json`:
...
Public `skills/` contains the same five QA Wolf skill directories as `plugins/qawolf/skills/`: `qawolf`, `qawolf-flow-outline`, `qawolf-flow-maintenance`, `qawolf-onboarding`, and `qawolf-trigger-setup`. Install them together as siblings, including the shared skill's `mcp.json` and references. Relative links connect the skills. Review existing files before updating; never replace an application's instructions or configuration wholesale.

## Portable clients with MCP

Install all five skills together, then configure MCP. The table shows the shared `qawolf` location; place `qawolf-flow-outline`, `qawolf-flow-maintenance`, `qawolf-onboarding` and `qawolf-trigger-setup` beside it. Clients supported by the [Agent Skills CLI](https://github.com/vercel-labs/skills) can select all five QA Wolf skills in its picker:
...
(
  ...
  dst=".agents/skills"
  for skill in qawolf qawolf-flow-outline qawolf-flow-maintenance qawolf-onboarding qawolf-trigger-setup; do
...
Install the five sibling skills under `.codewhale/skills/`, then run:
...
Install the five sibling skills under `.swival/skills/` or `.agents/skills/`. Configure `mcp_servers.qawolf` with `type = "http"` and `url`.
...
Use the original installer to update. For manual copies, review and replace only `qawolf`, `qawolf-flow-outline`, `qawolf-flow-maintenance`, `qawolf-onboarding`, and `qawolf-trigger-setup`. To uninstall, remove those skill directories, the QA Wolf instruction section, and its MCP entry. Preserve other skills, settings, and credentials.
